...
#hal_inst_t * halpr_find_inst_by_name(name)
cdef class Instance:
    ...
    def owner(self):
        ...
        with HALMutex():
            self._comp = halpr_find_comp_by_id(self._inst.owner_id)

        if self._comp == NULL:
            raise RuntimeError("BUG: Failed to find owning comp %d of instance %s: %s" %
                               (self._inst.owner_id, self._inst.name, hal_lasterror()))
    ...
    property owner_id:
        def __get__(self): return self._inst.owner_id
