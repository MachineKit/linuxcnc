...
static int instantiate(const int argc, const char**argv)
{
	...
	if (hal_pin_u32_newf(HAL_OUT, &brd->pins->irq_count,
			     inst_id, "%s.irq-count", name) ||
	    hal_pin_u32_newf(HAL_OUT, &brd->pins->irq_missed,
			     inst_id, "%s.irq-missed", name) ||
	    hal_pin_u32_newf(HAL_OUT, &brd->pins->write_errors,
			     inst_id, "%s.write-errors", name) ||
	    hal_pin_u32_newf(HAL_OUT, &brd->pins->read_errors,
			     inst_id, "%s.read-errors", name))
	    return -1;
	...
	if ((r = hal_export_xfunctf(&xfunct_args, "%s.waitirq", name)) != 0) {
