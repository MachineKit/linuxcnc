...
Usage:
           %(name)s [--compile -c|--preprocess -p|--document -d|--view-doc -v|--frontmatter -f] compfile...
    [sudo] %(name)s [--install -i|--install-doc -k] compfile..
...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "f:uicpdklo:h?",
